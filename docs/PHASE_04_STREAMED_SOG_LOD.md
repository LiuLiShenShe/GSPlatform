...
- [ ] 执行当前 `splat-transform --help` 并记录版本与可用参数。
- [ ] 阅读当前 Viewer fork 的 Streamed SOG 加载入口并记录准确模块路径。
- [ ] 确定使用上游原生分片、索引或 HTTP Range 中的哪一种真实机制。
- [ ] 在 ADR 中记录格式选择、版本兼容范围和回退方案。
- [ ] 不修改上游二进制格式；项目 manifest 仅包装业务元数据。

### B. 构建与发布脚本

- [ ] `scripts/build_streamed_sog.sh` 从归一化输入生成真实流式资产。
- [ ] 脚本使用锁定版本，启动时验证工具存在与版本。
- [ ] 生成过程失败时非零退出，不留下可被发布器误认的完整标记。
- [ ] 输出写入临时目录，校验成功后原子改名到发布目录。
- [ ] 生成 manifest、索引/分片、Poster、校验和清单和构建元数据。
- [ ] `scripts/verify_streamed_sog.sh` 能验证缺片、长度、哈希、索引边界和路径安全。
- [ ] 对同一输入与相同参数执行两次，结构与元数据可解释地一致。

### C. Stream scheduler

- [ ] 请求队列按可见性、屏幕贡献、相机距离和当前 LOD 排序。
- [ ] 低 LOD / 根节点始终优先于不可见高 LOD。
- [ ] 相机快速移动时取消或降低过时高质量请求优先级。
- [ ] 并发数、在途字节、解码队列和 GPU 上传有明确上限。
- [ ] 请求支持 AbortSignal，取消后不写入当前 session。
- [ ] 临时网络错误使用有上限的指数退避与抖动。
- [ ] 404、校验失败和格式失败不进行无限重试。

### D. LOD 选择

- [ ] 自动模式结合屏幕贡献、视锥、FPS、frame time 和内存预算。
- [ ] 省流模式降低预取和最大质量，并在 UI 明确显示。
- [ ] 高质量模式提高目标但仍受设备安全预算约束。
- [ ] 为阈值增加迟滞，避免相机轻微移动时频繁升降级。
- [ ] 相机静止后近处区域能逐步达到目标高质量。
- [ ] 远离或不可见区域能降级/回收，不无限累积。
- [ ] Quality 面板显示当前模式、目标 LOD、已驻留量和待加载量。

### E. 缓存与内存

- [ ] 网络响应进入有容量上限的 CPU LRU 缓存。
- [ ] 解码结果和 GPU 资源分别设置预算与淘汰策略。
- [ ] 缓存 key 包含资产版本或内容哈希，避免发布后读到旧分片。
- [ ] 同一分片的并发请求去重。
- [ ] 路由离开、sceneId 改变或 manifest 版本变化时正确失效。
- [ ] 长时间移动相机不会使 JS heap 和 GPU 资源无界增长。
- [ ] 内存压力下优先保持低 LOD 可交互并回收不可见高 LOD。

### F. HTTP 与 Nginx 行为

- [ ] 静态服务支持真实 Range 请求并返回正确 `206`、`Content-Range`、`Content-Length`。
- [ ] 无效 Range 返回 `416`。
- [ ] manifest 使用短缓存或版本化 URL；不可变分片使用内容哈希和长期 immutable 缓存。
- [ ] CORS 仅允许配置的前端来源与所需方法/请求头。
- [ ] HEAD 请求或等价元数据请求能得到正确长度。
- [ ] 压缩配置不破坏 Range 与 SOG 二进制内容。
- [ ] MIME 类型稳定，浏览器不会把二进制内容当文本执行。

### G. 进度与可观测性

- [ ] 总进度基于 manifest 中目标工作量和真实接收/解码/应用事件。
- [ ] 相机导致目标集合变化时，UI 区分“初始就绪”与“后台细化”。
- [ ] 100% 的定义明确：初始视图目标集合全部应用，而非整个无限浏览空间全下载。
- [ ] Performance 显示请求数、缓存命中、网络吞吐、解码队列、驻留 splat、FPS。
- [ ] 开发日志包含 session、asset version、chunk/range、耗时和错误码。
- [ ] 生产默认不输出高频逐帧日志。

### H. 测试与收尾

- [ ] 单元测试覆盖优先级、迟滞、取消、重试、LRU 和进度集合变化。
...
- [ ] 生成 `docs/reports/PHASE_04_REPORT.md`。
